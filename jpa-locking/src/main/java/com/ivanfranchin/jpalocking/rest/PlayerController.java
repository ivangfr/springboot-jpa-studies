...
public class PlayerController {
  ...
  public PlayerResponse playerCollectStars(
      @PathVariable Long id, @Valid @RequestBody StarCollectionRequest starCollectionRequest) {
    Player player = playerService.validateAndGetPlayer(id);
    log.info(
        "==> Player {} collects stars. Number of Stars: {}, Number of Lives: {}",
        id,
        player.getStars(),
        player.getLives());
    player = playerService.collectStars(player, starCollectionRequest.numStars());
    log.info(
        "<== Player {} collected stars. Number of Stars: {}, Number of Lives: {}",
        id,
        player.getStars(),
        player.getLives());
    return PlayerResponse.from(player);
  ...
  public PlayerResponse playerRedeemStars(@PathVariable Long id) {
    ...
    try {
      log.info(
          "==> Player {} redeems stars. Number of Stars: {}, Number of Lives: {}",
          id,
          player.getStars(),
          player.getLives());
      player = playerService.redeemStars(player);
      log.info(
          "<== Player {} redeemed stars. Number of Stars: {}, Number of Lives: {}",
          id,
          player.getStars(),
          player.getLives());
      return PlayerResponse.from(player);
      ...
      log.error(
          "An problem occurred while player {} redeems stars. Error class: {}, error message: {}",
          id,
          e.getClass().getName(),
          e.getMessage());
