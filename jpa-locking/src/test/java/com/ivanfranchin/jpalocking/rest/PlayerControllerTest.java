...
class PlayerControllerTest extends AbstractTestcontainers {
    ...
    void testStarsRedemption() throws InterruptedException {
        ...
        List<Callable<Void>> collectTasks = List.of(
        ...
        List<Callable<Void>> redeemTasks = List.of(
    ...
    private Callable<Void> collectStarsTask(CyclicBarrier barrier, Long playerId, int numStars,
    ...
    private Callable<Void> redeemStarsTask(CyclicBarrier barrier, Long playerId, AtomicInteger errorCounter) {
        return () -> {
            try {
                barrier.await();
